...
class AutoTransMpcLogger:
    ...
    def log_timer_cb(self, _event):
        ...
        with self.file_lock:
            if self.closed:
                return
            self.writer.writerow(setpoint_values + odom_values + actual_rpy + goal_values + reference_values +
                error_values + state_values + rc_values + force_values + applied_force_values + rpm_values +
                battery_values)
