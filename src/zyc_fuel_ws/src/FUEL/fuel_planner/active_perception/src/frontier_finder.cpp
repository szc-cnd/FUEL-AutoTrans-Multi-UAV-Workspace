...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, ros::NodeHandle& nh) {
  ...
  // 2026-07-08: 将近地前沿过滤改成可配置参数，避免在低空起飞和窄通道入口处把真实可探索前沿整批丢掉。
  nh.param("frontier/min_frontier_height", min_frontier_height_, 0.1);
...
void FrontierFinder::expandFrontier(
  ...
  while (!cell_queue.empty()) {
    ...
    for (auto nbr : nbrs) {
      ...
      // 2026-07-08: 低空雷达探索场景下不再硬编码 0.4m 截断，改由参数控制前沿最低高度。
      if (pos[2] < min_frontier_height_) continue;
      expanded.push_back(pos);
      cell_queue.push(nbr);
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  bool rebuild_cost_matrix = false;
  if (!removed_ids_.empty()) {
    // 2026-07-16: removed_ids_ 是按 frontiers_ 实时压缩后的下标，必须每次从 begin()+id 删除。
    // 旧代码复用单向迭代器，在连续/重复删除下标时会越过 end()，导致 free(): invalid pointer。
    for (auto it = frontiers_.begin(); it != first_new_ftr_; ++it) {
      if (it->costs_.size() != it->paths_.size()) {
        rebuild_cost_matrix = true;
        break;
      }
      for (const int removed_id : removed_ids_) {
        if (removed_id < 0 || static_cast<size_t>(removed_id) >= it->costs_.size()) {
          ROS_ERROR("[frontier_cost] inconsistent matrix row=%d size=%zu remove_id=%d; rebuild.",
                    it->id_, it->costs_.size(), removed_id);
          rebuild_cost_matrix = true;
          break;
        }
        auto cost_to_remove = it->costs_.begin();
        auto path_to_remove = it->paths_.begin();
        std::advance(cost_to_remove, removed_id);
        std::advance(path_to_remove, removed_id);
        it->costs_.erase(cost_to_remove);
        it->paths_.erase(path_to_remove);
      }
      if (rebuild_cost_matrix) break;
      std::cout << "(" << it->costs_.size() << "," << it->paths_.size() << "), ";
    }
    removed_ids_.clear();
  }
  if (rebuild_cost_matrix) {
    // 2026-07-16: 检测到历史矩阵不一致时放弃增量状态，从现有有效 frontier 全量重建。
    // 该分支只在异常时执行，避免为比赛常规规划引入持续的 O(N^2) 额外开销。
