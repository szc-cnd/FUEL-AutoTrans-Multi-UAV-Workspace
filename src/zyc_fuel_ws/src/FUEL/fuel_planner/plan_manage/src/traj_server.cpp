...
ros::Time last_record_time; // 用于控制数据记录频率
...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  ...
  if (start_time.isZero()) {
    ...
    // 初始化文件并写入表头
    if (!file_initialized) {
      flight_data_file.open("/home/guet3/zyc_fuel_ws/src/shiyan.txt", std::ios::out);
      if (flight_data_file.is_open()) {
        flight_data_file << "time,length,vel,energy" << std::endl;
        file_initialized = true;
        last_record_time = ros::Time::now();
        ROS_INFO("Flight data file initialized: shiyan.txt");
      } else {
        ROS_ERROR("Failed to open shiyan.txt file for writing");
      }
    }
  }
}
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  // 实时记录飞行数据(每1秒记录一次)
  if (file_initialized && !start_time.isZero()) {
    ros::Time time_now = ros::Time::now();
    double elapsed = (time_now - last_record_time).toSec();
    
    if (elapsed >= 1.0) { // 每1秒记录一次
      double len = calcPathLength(traj_cmd_);
      double flight_t = (time_now - start_time).toSec();
      double avg_vel = (len > 0 && flight_t > 0) ? (len / flight_t) : 0.0;
      
      flight_data_file << flight_t << "," << len << "," 
                      << avg_vel << "," << energy << std::endl;
      flight_data_file.flush(); // 确保数据立即写入文件
      
      last_record_time = time_now;
      
      // 同时在控制台输出
      ROS_INFO_THROTTLE(2, "time: %.2lf s, length: %.2lf m, vel: %.2lf m/s, energy: %.2lf",
                        flight_t, len, avg_vel, energy);
    }
  }
...
int main(int argc, char** argv) {
  ...
  traj_started_pub = node.advertise<std_msgs::Empty>("/planning/traj_started", 1, true);
  ...
  // 关闭文件
  if (file_initialized && flight_data_file.is_open()) {
