    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          // 2026-07-13: 竖直步代价高于水平步，禁止 A* 因三维距离近而习惯性翻越柱体。
          tmp_g_score = d_pos.x() * d_pos.x() + d_pos.y() * d_pos.y() +
                        vertical_weight_ * d_pos.z() * d_pos.z() + cur_node->g_score;
...
void Astar::setParam(ros::NodeHandle& nh) {
  ...
  // 2026-07-13: 默认仅改变路径偏好，不禁止必要的有限升降。
  nh.param("astar/vertical_weight", vertical_weight_, 4.0);
...
double Astar::getEuclHeu(Eigen::Vector3d x1, Eigen::Vector3d x2) {
  // 2026-07-13: 启发项同步惩罚高度差，否则高 lambda 下仅修改累计代价不会生效。
  const Eigen::Vector3d diff = x2 - x1;
  return tie_breaker_ *
         std::sqrt(diff.x() * diff.x() + diff.y() * diff.y() +
                   vertical_weight_ * diff.z() * diff.z());
}
