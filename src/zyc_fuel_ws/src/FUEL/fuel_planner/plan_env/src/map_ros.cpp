...
void MapROS::publishMapAll() {
  ...
  // 2026-07-09: /sdf_map 点云原来只写了 frame_id，没有写时间戳，导致下游把地图一直判成“远古过期数据”。
  // PCL 点云内部时间戳是 uint64，真正给 ROS 下游看的应当是 PointCloud2.header.stamp，这里在消息侧补齐。
  ...
  ofstream file("/home/guet3/zyc_fuel_ws/src/FUEL/fuel_planner/exploration_manager/resource/"
                "curve1.txt",
                ios::app);
...
void MapROS::publishMapLocal() {
  ...
  // 2026-07-09: occupancy_local 的 ROS 消息必须带仿真时间，否则前置找门阶段会一直把地图当成 stale。
  cloud_msg.header.stamp = ros::Time::now();
  map_local_pub_.publish(cloud_msg);
  pcl::toROSMsg(cloud2, cloud_msg);
  // 2026-07-09: occupancy_local_inflate 与 occupancy_local 保持同样的有效时间戳，避免门检测读到“零时刻”地图。
  cloud_msg.header.stamp = ros::Time::now();
...
void MapROS::publishUnknown() {
  ...
  sensor_msgs::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  // 2026-07-09: unknown 地图也补齐 ROS 时间戳，避免入口检测把未知区信息判成过期。
  cloud_msg.header.stamp = ros::Time::now();
...
void MapROS::publishDepth() {
  ...
  sensor_msgs::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  // 2026-07-09: depth 可视化消息同步带当前时刻，避免调试时不同地图源时间基不一致。
  cloud_msg.header.stamp = ros::Time::now();
...
void MapROS::publishESDF() {
  ...
  sensor_msgs::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  // 2026-07-09: ESDF 可视化也统一写 ROS header.stamp，保持整条地图链路的时序一致。
  cloud_msg.header.stamp = ros::Time::now();
