...
该包统一接收颜色标签、二维码和热源检测结果，通过 ROS TF 转换到 FAST-LIO 的 `camera_init` 世界坐标系，并将其作为比赛语义上的 `channel` 坐标。三个检测器都会先发布原始候选，随后由各自的时间稳定/真实性校验产生 `stable`、`confirmable` 状态；候选立即生成机载端实时观测并显示在 RViz，但只有通过确认门控后才登记、保存证据图并发送远程端。颜色标签和二维码沿用检测器自身门控；热源还需在世界坐标附近连续命中 20 次（20 Hz 下约 1 秒），用于过滤移动人员。当前阶段候选和确认结果都不接入规划器。
...
视野边缘的热点也允许确认。随后还需在世界坐标 0.30 m 范围内连续命中 20 次，
相邻观测中断超过 0.25 秒就重新累计，以过滤移动人员。
