...
保存证据时，通用信息写入原图下方新增的区域，不覆盖检测画面。热源的 D435
映射图已经包含状态、映射框、深度和坐标，因此保持原始 1280×720 尺寸直接保存，
不再重复添加证据信息栏。
...
5 帧。热源先通过 3 帧窗口内至少 2 次且像素跳变不超过配置阈值；贴住 ROI
边缘的不完整热点只作为候选。随后还需在世界坐标 0.30 m 范围内连续命中 20 次，
相邻观测中断超过 0.25 秒就重新累计。
