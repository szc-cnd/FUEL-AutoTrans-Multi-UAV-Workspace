# 三类目标检测—规划闭环改动说明（历史备份）

> 当前状态（2026-08-10）：本文件第 1～9 节记录此前做过的“检测—规划闭环”方案，仅作为备份参考。按当前需求，规划器侧闭环接入已经回退，FUEL 不再订阅候选结果、不再生成检测观察位姿；当前只保留检测包、远程上报和 RViz 显示。回退前完整版本已在各仓库创建标签 `backup/detection-planning-uav0-20260810`。
...
| `match_ws` | `132aed2` | 回退检测规划闭环，恢复规划器原有逻辑 |
| `target_reporting` | `b686485` | 增加 RViz 检测标记、图像配置和独立启动文件 |

`match_ws` 中原有的 FAST-LIO 参数、视频、PDF 和临时备份文件没有纳入本次提交。

## 10. 当前只显示检测结果的启动方式

先启动 FAST-LIO、三个检测节点和目标上报节点：
...
RViz 订阅 `/UAV0/target_reporting/markers`（`visualization_msgs/MarkerArray`），固定坐标系为 `world`。黄色表示候选，绿色表示确认；三个调试图像也已写入 `target_results.rviz`。该显示链路不发布任何规划目标，不改变 FUEL 的出口、避障或降落逻辑。
