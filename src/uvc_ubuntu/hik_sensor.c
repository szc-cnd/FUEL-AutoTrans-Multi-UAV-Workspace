...
#define STREAM_TYPE_YUV_ONLY      10
...
static int image_manual_correct(uvc_t *u, int retries)
{
unsigned char data[1] = {0};

printf("\n--- starting image_manual_correct ----\n");

/* IMAGE_MANUAL_CORRECT: Data[0] = channelID, existing code uses channelID = 1 */
data[0] = 1;

if (set_curr_func(u, XU_CS_ID_IMAGE, IMAGE_MANUAL_CORRECT, retries) != 0)
return -1;
...
/* 手动校正需要一点执行时间 */
wait_cmd_done(u, 300, 300);

printf("--- image_manual_correct done ----\n");
...
if (image_manual_correct(u, 20) != 0) {
printf("\n--- image_manual_correct error ---\n");
}
wait_cmd_done(u, 300, 300);
...
if (image_enhance_config(u, 10, 40) != 0) {
	...
	/*
	 * Competition detection uses streamType=10: YUV-only without a
	 * temperature header. Do not parse temperature matrices or switch to
	 * the full-screen temperature + YUV combined stream.
	 */
	if (stream_type_config(u, STREAM_TYPE_YUV_ONLY, 20) != 0) {
