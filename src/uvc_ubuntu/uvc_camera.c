...
void  uvc_set_video_mode(uvc_t *u, int *w, int *h, int *fps)
{
	int   width, height = 0;
	int   offset_fix = 0;
	const char *offset_fix_env = getenv("HIK_YUV_OFFSET_FIX");
	...
	*fps = 10000000/frame_desc->dwDefaultFrameInterval;
	...
	u->size = width * height * 2;
	if (offset_fix_env && *offset_fix_env) {
		offset_fix = atoi(offset_fix_env);
	}
	/* streamType=10 is pure YUYV without a temperature header. */
	u->offset = offset_fix;

	printf("req_width=%d,req_height=%d,size=%d,len=%d,offset=%d,offset_fix=%d\n",
	       width, height, u->size, u->len, u->offset, offset_fix);
