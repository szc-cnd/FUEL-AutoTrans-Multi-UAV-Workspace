...
本文档是 CAV0 的实施设计与执行基线。代码、消息和 launch 的实际改动以
`/home/asus/match_ws` 中的源文件为准；本文档不替代实飞前的链路检查。

目标是在 CAV0 的 `/home/asus/match_ws` 中保留 UAV0 前机的 FUEL 规划器，用
AutoTrans MPC 替换原来的 `px4ctrl/cxr_egoctrl` 控制器，同时保留原控制器作为可回退入口。
...
- `has_yaw=false` 或 yaw 数据无效时，保持原有 yaw 回退逻辑，不影响旧轨迹。
- CAV0 的 FUEL 桥接发送的消息将 `has_yaw=true`，并保留 FUEL 的 yaw/yaw_rate。
- 旧 Diff-Planner bridge 仍可保留，但不作为本次 UAV0 FUEL 接入的输入链路。
