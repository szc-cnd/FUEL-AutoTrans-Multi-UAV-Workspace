...
高度到 `≤1.50 m` 后立即记录当前融合定位 X/Y 并进入 `FIXED_XY_DESCENT`，不再执行
0.08 m、0.5 秒的二次精确对准；固定 X/Y 下降到估计离地高度 `≤0.30 m` 后再切
`AUTO.LAND`。
...
完整的一键启动、六终端分别启动、人工触发、查询和停止流程已移到
[`搜索降落独立测试_UAV0.md`](搜索降落独立测试_UAV0.md)，主比赛流程不再重复维护
这套专项测试命令。
