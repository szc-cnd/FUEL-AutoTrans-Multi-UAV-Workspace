...
def rotate_covariance(rotation, covariance):
    ...
    source = tuple(
        tuple(covariance[3 * row + column] for column in range(3))
        for row in range(3)
    )
    left = tuple(
        tuple(
            sum(rotation[row][k] * source[k][column] for k in range(3))
    ...
    result = tuple(
        tuple(
            sum(left[row][k] * rotation[column][k] for k in range(3))
            for column in range(3)
        )
        for row in range(3)
    )
...
def main():
    ...
    input_topic = rospy.get_param("~input_topic", "/UAV0/livox/imu")
    output_topic = rospy.get_param("~output_topic", "/UAV0/livox/imu_body")
    output_frame_id = rospy.get_param("~output_frame_id", "UAV0/body")
